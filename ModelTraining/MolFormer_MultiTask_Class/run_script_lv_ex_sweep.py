...
# hyperparameters
task_name_list = ['bird', 'cat', 'chicken', 'dog', 'duck', 'gpig', 'mammal', 'man', 'mouse', 'quail', 'rabbit', 'rat', 'woman'] # no 'human'
...
def train_model(config=None):
    ...
    wandb.init( project = PROJECT,
                group = "extask",
                notes = "testing extask and timing",
                config = config,
    )
    config = wandb.config
    ...
    directory = Path(f'oral_data')
    ex_path = Path(args.exdataset) # path of dataset to exculse
    num_tasks = len(list(directory.iterdir())) - 1 # excluding 1 task
